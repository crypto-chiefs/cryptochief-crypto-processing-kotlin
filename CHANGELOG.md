...
- **Breaking for direct callers of `RequestSigner.signHmacV1()`:** it now returns the full `X-CC-Signature` header value (`v1=` + lowercase hex), matching `RequestSigner.signWebhookV1()`. Requests sent through the client are unchanged — the header on the wire is byte-for-byte the same
## [0.11.0] — 2026-09-19

The fee of a transfer without signing it, TRON energy for rent, native coin for
sale against API credits — and an order-shaped answer to the failures that used
to arrive as bare exceptions.

- `client.transactions.estimate()` — `POST /v1/transaction/estimate`: the network fee of a native or token transfer, with nothing signed and nothing broadcast. `EstimateTransactionRequest` is `SignTransactionRequest` minus `url_callback` and `calls`; the answer carries `estimatedFee` and `required` — all the native coin the from-wallet must hold: fee plus `value` for a native transfer, the fee alone for a token — each with a USD twin that is empty when no rate is available. The call itself is billed to the credits balance: 10 000 credits (0.001 USD) per estimate. `type = contract` is refused with `CONTRACT_ESTIMATE_UNSUPPORTED`
- On TRON the estimate also carries a fee breakdown: `feeExpected` (what the fee is expected to be with the wallet's current energy pool — staked, delegated, rented — counted in; not a guarantee, the pool can run out before the transaction is broadcast and the burn falls back to `estimatedFee`), `feeLimit` (the on-chain cap written into the transaction), `energy` (units needed) and `energyFee` + `bandwidthFee` + `activationFee`, which add up to the gross `estimatedFee`; `activationFee` appears only on a native transfer to a not-yet-activated address. On every other network these fields are absent — `null`, never `""`
- An infrastructure failure of the estimate — the scanner or the fee source down — arrives as a 502 with `ApiException.code` `ESTIMATE_UNAVAILABLE`: retryable, unlike a 400 refusal, which a repeat will not change
- `client.energy` — TRON energy rental, billed to the same credits balance as the rest of the API. `quote()` (`POST /v1/energy/quote`) prices one rent against burning TRX for the same transfer — free, no paid call, nothing billed; `rent()` (`POST /v1/energy/rent`) places it, synchronous: by the time it returns, the energy is delegated to `receiveAddress` — the SENDER of the planned transfer — or the refusal is known; `order(key)` reads a rent back by its idempotency key. Omitted `energy`/`durationSec` are sized by the platform to one transfer for the address
- `client.native` — buying the native coin of a network (TRX, ETH, BNB, SOL, TON, ...) from the platform's liquidity, billed to API credits. The price already includes the fee of the platform's own transfer to the receiving address, so `receiveAddress` can be any address; `totalUsd` is the full sale price and `credits` the exact amount the buy bills. `quote()` (`POST /v1/native/quote`) — free, holds about 90 seconds, single-use; `buy()` (`POST /v1/native/buy`) takes the three fields or an unexpired `quoteRef`; `order(key)` reads a buy back
- `energy.rent()` and `native.buy()` require an `Idempotency-Key` — the server refuses the call with 400 without one, and the SDK refuses it earlier with `IllegalArgumentException`. Set it as for a payout execute: `withIdempotencyKey("energy-...") { client.energy.rent(...) }`. The key deduplicates the operation: a repeat with the same key reads the same order back instead of billing twice, so a retry after a network failure or a 502 is safe
- A refused or unresolved rent/buy comes back as the order, not as an exception: the API answers a business outcome with the order itself as the body — 402 when the credits balance is short (`errorCode` `INSUFFICIENT_CREDITS`, nothing billed), 502 for a `refused` order, 409 for an `unresolved` one (`needsAttention = true` — do NOT retry: the energy or coins may already be on their way; follow the order with `order()` until it settles) — and the services decode and return it. `EnergyOrder` and `NativeOrder` carry `error` (human text) and `errorCode` (the machine code to branch on), plus `isTerminal`, `succeeded` and the `EnergyOrderStatus` / `NativeOrderStatus` constants; on a `refused` order the billing fields (`priceUsd`, `credits`, `trxUsd`; `txHash`, `totalUsd`, ...) are `null`. A non-2xx with no order to report — a `{"ok":false,...}` envelope like an expired or already-used quote (`QUOTE_EXPIRED` / `QUOTE_ALREADY_USED`), a gateway error — still throws `ApiException`
- `ApiException.code` folds the order-body error shape too: on a non-2xx body that carries it, `error_code` is the machine code and `error` the sanitized human text, alongside the gateway and white-label envelopes it already read
- Repository hygiene the 0.10.0 entry did not record: `.gitattributes` pins text checkouts to LF on every platform, Windows included — the signature test vectors are pinned by sha256, and a CRLF checkout changed their bytes, turning a green suite red for no code reason — and a CI workflow now runs the suite on every push
...
- `client.webhooks.info(uuid)` — `POST /v1/webhooks/info`. One delivery the platform made to your endpoint: its status, every attempt with the HTTP status, duration and **the body your endpoint answered** (capped; `responseTruncated` says if it was cut), and the payload that was sent. `null` on a field means "not recorded" — an attempt nothing answered has a `null` `httpStatus` and the transport error in `error`
- `client.webhooks.resend(uuid)` — `POST /v1/webhooks/resend`, re-fires one delivery right now. On this platform the resend is **synchronous**: the POST happens before the answer, so `queued == true` arrives with `status` already `delivered` or `failed` for that attempt
- `client.webhooks.resendStaticDeposit(depositUuid)` — `POST /v1/static-deposits/resend`, re-fires the **newest** webhook of a static deposit by the deposit's own uuid, for when you hold the deposit and not the delivery. Older events of the deposit are superseded and are not resent. Same routes, bodies and refusal codes as the white-label platform, so one integration runs against either
- `WebhookVerifier.DELIVERY_HEADER` (`X-Webhook-Delivery`) — the delivery uuid now rides on every webhook. Constant across attempts and resends of one delivery, so it doubles as your receiver's idempotency key, and it is the argument the new methods take. **Keep it when you log a webhook** — the API has no listing of deliveries, so it is the only way to name one afterwards
- `ErrorCode.DELIVERY_SUPERSEDED`, `DELIVERY_IN_FLIGHT`, `RESEND_TOO_SOON`, `NO_DELIVERIES`, `NOT_FOUND`. A superseded delivery — one with a newer event for the same object — cannot be resent: re-sending `invoice.in_mempool` after `invoice.paid` would tell your system the order went backwards. The exception names the newer event; resend that
